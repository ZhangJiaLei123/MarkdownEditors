...
package com.blxt.markdowneditors.presenter;
...
public class FolderManagerPresenter extends BasePresenter<IFolderManagerView> {
    ...
    public boolean rename(FileBean bean, String targetName) {
        if (bean == null || Check.isEmpty(targetName)) {
            return false;
        }
        ...
        //更新列表
        if (getMvpView() != null) {
            getMvpView().updatePosition(files.indexOf(bean), bean);
        }
